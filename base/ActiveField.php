...
class ActiveField extends Component
{
	 ...
	 * 自动完成
	 */
	public function autocomplete($options = [],$keyValue=true,$url='autocomplete'){
		if (isset($options['label']) && !isset($this->parts['{label}'])) {
			$this->parts['{label}'] = $options['label'];
			if (!empty($options['labelOptions'])) {
				$this->labelOptions = $options['labelOptions'];
		...
		unset($options['labelOptions']);
		$options['label'] = null;
		$this->parts['{input}'] = \fufudao\base\AutoComplete::widget(['model' =>$this->model, 
	 ...
	 * @return type
	 */
	public function select($data, $htmlOptions=[],$selectValue=null){
		if (isset($htmlOptions['label']) && !isset($this->parts['{label}'])) {
			$this->parts['{label}'] = $htmlOptions['label'];
			if (!empty($htmlOptions['labelOptions'])) {
				$this->labelOptions = $htmlOptions['labelOptions'];
			}
		}
		unset($htmlOptions['labelOptions']);
		$htmlOptions['label'] = null;
		
		
		$model=  $this->model;
		$attribute = $this->attribute;
//		self::$count++;
		$selectionText = Yii::t('sys','unset');
		if(isset($htmlOptions['empty']) && empty($htmlOptions['empty'])){
			$htmlOptions['empty'] =$selectionText;
		}
		if( empty($htmlOptions['name']))
			$htmlOptions['name'] =Html::getInputName($this->model, $this->attribute);
		if( empty($htmlOptions['id']))
			$htmlOptions['id'] = 'select-'.$this->attribute;
		...
		if( isset($selectValue) )
			...
			$selection = ArrayHelper::getValue($model,$attribute);

		$content="\n".  $this->listSelections($selection,$data,$htmlOptions);
		...
		if(!is_array($selection) && isset($selection) && isset($data[$selection])){
			$selectionText = $data[$selection];
		}
		
		
		$content ='<span>'.$selectionText.'</span>'.Html::tag('ul',$content,array('class'=>'hide'));
		
		if( !isset($htmlOptions['readonly']))
			$content.='<span class="dropdown"><i class="iconfont small-btn">&#xe651;</i></span>';
		
		
//		if( empty($htmlOptions['class']))$htmlOptions['class']='select';
		$htmlOptions['class'] = empty($htmlOptions['class'])? 'select':'select '.$htmlOptions['class'];
		
		$this->parts['{input}'] =Html::tag('div',$content,$htmlOptions);
