    ...
    public function registerClientScript()
    {
        ...
        if ($this->linkSelector !== false) {
			...
			if(isset($this->target))
				$js .= "jQuery(document).pjax($linkSelector, \"#{$this->target}\", $options);";
			else
				$js .= "jQuery(document).pjax($linkSelector, \"#$id\", $options);";
        }
        if ($this->formSelector !== false) {
            $formSelector = Json::htmlEncode($this->formSelector !== null ? $this->formSelector : '#' . $id . ' form[data-pjax]');
			$targetId = isset($this->formTarget)?$this->formTarget:$id;
            $js .= "\njQuery(document).on('submit', $formSelector, function (event) {jQuery.pjax.submit(event, '#$targetId', $options);});";
        }
        $view = $this->getView();
